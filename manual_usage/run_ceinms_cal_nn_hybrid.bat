...
set "CALIBRATION_SETUP=%SCRIPT_DIR%calibrationSetup.xml"
...
if defined OUTPUT_DIR (
    ...
    echo Warning: Could not find tag <outputDirectory> tag in "%CALIBRATION_SETUP%".
    pause
    exit /b 1
)
...
if %errorlevel% neq 0 (
    echo Error running CEINMS calibration.
    exit /b %errorlevel%
)

rem Display success message and path to output directory
echo CEINMS calibration completed successfully.
echo Output directory: %OUTPUT_DIR%
echo Output subject file: %OUTPUT_DIR%\subjectCalibrated_hybrid.xml
pause
