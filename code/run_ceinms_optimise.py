...
def main(optimise_setup=None):
    ...
    log_file_path = os.path.join(output_dir, "optimise.log")
...
if __name__ == "__main__":
    ...
    ceinms_setup = input("Enter path to CEINMS optimise setup XML file: ").strip()
    
    if not os.path.isfile(ceinms_setup):
        raise FileNotFoundError(f"CEINMS optimise setup file not found at {ceinms_setup}")

    # Run CEINMS optimise
    try:
        main(optimise_setup=ceinms_setup)
